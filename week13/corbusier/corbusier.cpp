...
typedef std::vector<vec_bb> vec_bbb;

bool dp(int h, int d, int num_disks, const int i, const int k, const vec_l &heights, vec_bbb &memo_init, vec_bbb &memo) {
    if (h == i && num_disks > 0) {
    ...
    if (memo_init[d][num_disks][h]) return memo[d][num_disks][h]; // check memo

    // try taking current disk
    if (dp((h + heights[d]) % k, d + 1, num_disks + 1, i, k, heights, memo_init, memo)) {
        memo_init[d][num_disks][h] = true;
        memo[d][num_disks][h] = true;
...
void testcase() {
    ...
    vec_bbb memo_init(n, vec_bb(n, vec_b(k, false)));
    vec_bbb memo(n, vec_bb(n, vec_b(k, false)));
