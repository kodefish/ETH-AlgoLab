...
void testcase() {
    ...
    for (int i = 0; i < S; i++) {
        ...
        eaG.addEdge(states + i, v_target, state_limit, 0);
    }

    // Read in all the sites and link them to their state
    for (int i = 0; i < M; i++) {
        int state; std::cin >> state;
        eaG.addEdge(sites + i, states + state - 1, 1, 0);
    }
